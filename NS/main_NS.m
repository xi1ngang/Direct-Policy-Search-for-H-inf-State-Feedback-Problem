clear;
clc;
% This script implemented the nonderivative gradient sampling
% algorithm, the cruve of NGS in Figure 1
...
epsilon = epsilon0;
K = K0;
Num = 100; % Maximum iteration number
KK = K0;
T = [];
Delta = [];
JJJ = comput_Hinf(A,B,K0,Q,R,n,p); %Initial cost function value
% Algorithm loop
for N = 1:Num
    K_samples = randsphere(m,n*p,delta)+reshape(K,1,n*p).*ones(m,1);
    P = gradient_est(K_samples,A,B,Q,R,alpha,n,p);
